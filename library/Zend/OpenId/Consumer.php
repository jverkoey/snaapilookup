...
class Zend_OpenId_Consumer
{
    ...
    public function verify($params, &$identity = "", $extensions = null)
    {
        ...
        if (empty($params['openid_mode'])) {
            $this->_setError(msg_dev_only("Missing openid.mode"));
            return false;
        }
        if (empty($params['openid_return_to'])) {
            $this->_setError(msg_dev_only("Missing openid.return_to"));
            return false;
        }
        if (empty($params['openid_signed'])) {
            $this->_setError(msg_dev_only("Missing openid.signed"));
            return false;
        }
        if (empty($params['openid_sig'])) {
            $this->_setError(msg_dev_only("Missing openid.sig"));
            return false;
        }
        if ($params['openid_mode'] != 'id_res') {
            $this->_setError(msg_dev_only("Wrong openid.mode '".$params['openid_mode']."' != 'id_res'"));
            return false;
        }
        if (empty($params['openid_assoc_handle'])) {
            $this->_setError(msg_dev_only("Missing openid.assoc_handle"));
            return false;
        }
        if ($params['openid_return_to'] != Zend_OpenId::selfUrl()) {
            ...
            if ($pos === false ||
                ...
                $this->_setError(msg_dev_only("Wrong openid.return_to '".
        ...
        if ($version >= 2.0) {
            if (empty($params['openid_response_nonce'])) {
                $this->_setError(msg_dev_only("Missing openid.response_nonce"));
                return false;
            }
            if (empty($params['openid_op_endpoint'])) {
                $this->_setError(msg_dev_only("Missing openid.op_endpoint"));
                return false;
            /* OpenID 2.0 (11.3) Checking the Nonce */
                ...
                $this->_setError(msg_dev_only("Duplicate openid.response_nonce"));
                return false;
            }
        }
            ...
            if (base64_decode($params['openid_sig']) ==
                ...
                if (isset($params['openid_claimed_id'])) {
                    ...
                    if (!Zend_OpenId::normalize($id)) {
                        ...
                        $this->_setError("We can't establish a connection to that OpenID. Check that you've typed in your username correctly.");
                        return false;
    ...
    protected function _httpRequest($url, $method = 'GET', array $params = array(), &$status = null)
    {
        ...
        try {
            ...
            $this->_setError('HTTP Request failed: ' . $url . ' ' . $e->getMessage());
            return false;
        }
    ...
    protected function _discovery(&$id, &$server, &$version)
    {
        ...
        if (preg_match(
            ...
            $this->_setError('Unable to match against any regexes with id "' . $id . '"<pre>' . $response . '</pre>');
            return false;
        }
        if ($version >= 2.0) {
            if (preg_match(
